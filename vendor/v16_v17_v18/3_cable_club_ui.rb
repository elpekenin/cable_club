class CableClub_Scene
  ...
  def pbTeamPreview(partner_trainer,partner_party,timer)
    dummy_trainer = PokeBattle_Trainer.new($Trainer.name,$Trainer.online_trainer_type)
    pbFadeOutIn(99999){
...
class CableClubScreen
  ...
  def pbAttemptConnection
    ...
    begin
      msg = _ISPRINTF("What's the ID of the trainer you're searching for? (Your ID: {1:05d})",$Trainer.publicID($Trainer.id))
      partner_id = ""
      loop do
      ...
      begin
        ...
        case res
        when 0; pbConnectServer(partner_id)
  ...
  def await_server(connection,partner_id)
    change_state(:await_server){
      if connection.can_send?
        ...
        pbDisplayDots(_ISPRINTF("Your ID: {1:05d}\nConnecting",$Trainer.publicID($Trainer.id)))
      end
    }
  ...
  def await_partner(connection)
    ...
    change_state(:await_partner){  
      pbDisplayDots(_ISPRINTF("Your ID: {1:05d}\nSearching",$Trainer.publicID($Trainer.id)))
      connection.update do |record|
        case (type = record.sym)
        when :found
          @client_id = record.int
          @partner_name = record.str
          @partner_trainertype = record.int
  ...
  def battle_check_team_preview(connection)
    ...
    if battle_rules.team_preview?
      partner = PokeBattle_Trainer.new(@partner_name, @partner_trainertype)
  ...
  def do_battle(connection,team_order,partner_order)
    partner = PokeBattle_Trainer.new(@partner_name, @partner_trainertype)
    partner.online_win_text = @partner_win_text
    partner.online_lose_text = @partner_win_text
  ...
  def confirm_trade_pokemon(connection)
    ...
    if @partner_chosen>=0
      ...
      able_party=@partner_party.find_all { |p| p && !p.isEgg? && p.hp>0 }
...
class TeamPreview_Scene
  ...
  def pbStartScene(left_trainer,left_party,right_trainer,right_party,timer)
    ...
    left_party.each_with_index do |pkmn, i|
      ...
      if !defined?(ESSENTIALSVERSION) && !defined?(ESSENTIALS_VERSION)
        @sprites["item_l#{i}"] = ChangelingSprite.new((base_x + 72),(base_y+96),@viewport)
        @sprites["item_l#{i}"].addBitmap("itembitmap","Graphics/Pictures/item")
        @sprites["item_l#{i}"].addBitmap("mailbitmap","Graphics/Pictures/mail")
        @sprites["item_l#{i}"].visible=(pkmn.item>0)
        if @sprites["item_l#{i}"].visible
          @sprites["item_l#{i}"].changeBitmap(pkmn.mail ? "mailbitmap" : "itembitmap")
        end
      else
    ...
    right_party.each_with_index do |pkmn, i|
      ...
      if !defined?(ESSENTIALSVERSION) && !defined?(ESSENTIALS_VERSION)
        @sprites["item_r#{i}"] = ChangelingSprite.new((base_x + 72),(base_y+96),@viewport)
        @sprites["item_r#{i}"].addBitmap("itembitmap","Graphics/Pictures/item")
        @sprites["item_r#{i}"].addBitmap("mailbitmap","Graphics/Pictures/mail")
        @sprites["item_r#{i}"].visible=(pkmn.item>0)
        if @sprites["item_r#{i}"].visible
          @sprites["item_r#{i}"].changeBitmap(pkmn.mail ? "mailbitmap" : "itembitmap")
        end
      else
  ...
  def pbDrawTeamPreviewText(left_name,right_name,left_party,right_party)
    ...
    text_pos = [
      [left_name,114,16,2,baseColor,shadowColor],
      [right_name,398,16,2,baseColor,shadowColor],
    ]
    left_party.each_with_index do |pkmn, i|
      base_x = ((i%2)*128)
      base_y = ((i/2)*80)
      text_pos.push([pkmn.name,(base_x+64),(base_y+116),2,baseColor,shadowColor])
      if pkmn.isMale?
        text_pos.push([_INTL("♂"),(base_x+96),(base_y+64),false,Color.new(0,112,248),Color.new(120,184,232)])
      elsif pkmn.isFemale?
        text_pos.push([_INTL("♀"),(base_x+96),(base_y+64),false,Color.new(232,32,16),Color.new(248,168,184)])
      end
    end
    right_party.each_with_index do |pkmn, i|
      base_x = ((i%2)*128)
      base_y = ((i/2)*80)
      text_pos.push([pkmn.name,(base_x+320),(base_y+116),2,baseColor,shadowColor])
      if pkmn.isMale?
        text_pos.push([_INTL("♂"),(base_x+350),(base_y+64),false,Color.new(0,112,248),Color.new(120,184,232)])
      elsif pkmn.isFemale?
        text_pos.push([_INTL("♀"),(base_x+350),(base_y+64),false,Color.new(232,32,16),Color.new(248,168,184)])
      end
    end
